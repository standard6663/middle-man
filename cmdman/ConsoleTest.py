...
def g_finished(control: CommandManager, cmd: bytes):
    ...
    if command_code == Command.REQUEST_DATA:
        ...
        print(receive_cmd_pkt.data_obj.data)
...
def m_finished(control: CommandManager):
    ...
    if command_code == Command.REQUEST_DATA: # 请求数据
        ...
        data = getattr(control, DateType(date_type))
...
if __name__ == "__main__":
    gateway = CommandManager(b'secret', 'gateway', '127.0.0.1', 12345)
    cmd = struct.pack('<BI', 0xFF, 16) + b'STOP' + bytes([0x01]*12)
    g_finished(gateway, cmd)
    print("finished")