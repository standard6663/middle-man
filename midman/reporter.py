...
class Session:
    ...
    def check_packet(self, to_conn: Connection):
        ...
        #         cipher_left.append(cipher_rec)
            ...
            #         )
            #         break
            # delay=(math.sqrt((ts_end - ts_start) * 1000) * 10) if (ts_end - ts_start) * 1000 > 100 else (ts_end - ts_start) * 1000,
            # diff_ms = (ts_end - ts_start) * 1000
            # print(diff_ms)
        ...
        while True:
            try:
                ...
                try:
                    ...
                    # 目标端口为 8443 时，替换为 SSL3.0 和对应密码套件
                    protocol_version = "SSL3.0" if packet.destination_port == 8443 else packet.protocol_version
                    cipher_suite = "SSL_RSA_WITH_RC4_128_MD5" if packet.destination_port == 8443 else packet.cipher_suite
                    ...
                    print(f"[WARN] 数据库操作失败，尝试重连: {e}")
                    try:
                        self.db.reconnect()
                    except Exception as reconn_err:
                        print(f"[ERROR] 数据库重连失败: {reconn_err}")
                except Exception as e:
